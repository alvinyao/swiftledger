...
import com.higgschain.trust.consensus.core.master.command.ArtificialChangeMasterCommand;
import com.higgschain.trust.consensus.core.master.command.ChangeMasterCommand;
import com.higgschain.trust.consensus.core.master.command.ChangeMasterVerifyResponse;
import com.higgschain.trust.consensus.core.master.command.MasterHeartbeatCommand;
...
@Slf4j @Component @Replicator public class MasterCommandReplicate {
    ...
    public void changeMaster(ConsensusCommit<ChangeMasterCommand> commit) {
        ...
        if (operation.getTerm() == nodeState.getCurrentTerm() + 1) {
            ...
            if (collect.size() >= currentView.getAppliedQuorum()) {
                ...
                ITermManager.startNewTerm(operation.getTerm(), operation.getMasterName());
            }
        }
        commit.close();
    }
    ...
    public void artificialChangeMaster(ConsensusCommit<ArtificialChangeMasterCommand> commit) {
        ...
        ITermManager.getTerms().clear();
        ITermManager.startNewTerm(operation.getTerm(), operation.getMasterName(), operation.getStartHeight());
        commit.close();
    }
