...
public class VM {
    ...
    private static final Map<OpCode, Function<BlockChainConfig, Boolean>> OP_VALIDATORS = new HashMap<OpCode, Function<BlockChainConfig, Boolean>>() {{
        put(OpCode.DELEGATECALL, (config) -> config.getConstants().hasDelegateCallOpcode());
        put(OpCode.REVERT, BlockChainConfig::eip206);
        put(OpCode.RETURNDATACOPY, BlockChainConfig::eip211);
        put(OpCode.RETURNDATASIZE, BlockChainConfig::eip211);
        put(OpCode.STATICCALL, BlockChainConfig::eip214);
    }};
    ...
    public String stepInto(final OpCode op, final Program program,  final BlockChainConfig blockChainConfig,
        ...
        switch (op) {
            ...
            case PUSH32: {
                ...
                if ( LOGGER.isInfoEnabled()) {
                    hint = "" + ByteUtil.toHexString(data);
                }
                ...
                case SUICIDE:
                    ...
                    for (DataWord key : storageKeys) {
                        DUMP_LOGGER.trace("{} {}",
                                ...
                                ByteUtil.toHexString(details.getStorage().get(key).getNoLeadZeroesData()));
                    }
                default:
                    break;
            }
            String addressString = ByteUtil.toHexString(program.getOwnerAddress().getLast20Bytes());
            String pcString = ByteUtil.toHexString(new DataWord(program.getPC()).getNoLeadZeroesData());
            String opString = ByteUtil.toHexString(new byte[]{op.val()});
            String gasString = ByteUtil.toHexString(program.getGas().getNoLeadZeroesData());
