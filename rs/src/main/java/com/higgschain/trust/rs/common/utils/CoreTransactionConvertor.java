    ...
    public String buildContractCode(String sourceCode, String contractor,String policyId, Object... contractInitArgs) {
        try {
            if(!StringUtils.isEmpty(policyId) && contractInitArgs != null){
                ...
                Object[] args = new Object[contractInitArgs.length+1];
                for(int i=0;i<contractInitArgs.length;i++){
                    args[i] = contractInitArgs[i];
                }
                args[args.length-1] = policyIdHexString;
                contractInitArgs = args;
            }
