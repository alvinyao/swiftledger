...
import com.higgschain.trust.network.message.*;
...
public class MessagingService {
    ...
    public CompletableFuture<MessagingService> start() {
        ...
        return trustServer.start().thenRun(() -> {
            timeoutExecutor = Executors.newSingleThreadScheduledExecutor(Threads.namedThreads("messaging-timeout-%d", log));
    ...
    private void initEventLoopGroup() {
        clientGroup = new NioEventLoopGroup(config.clientThreadNum(), Threads.namedThreads("netty-network-event-nio-client-%d", log));
        serverGroup = new NioEventLoopGroup(0, Threads.namedThreads("netty-network-event-nio-server-%d", log));
    }
                ...
                if (message.isRequest()) {
                    ...
                    if (NetworkRequest.AUTH_ACTION_NAME.equals(networkRequest.actionName())) {
                        ...
                        if (req instanceof AuthenticationRequest) {
                            ...
                            newPeer.setBackup(request.isBackupNode());
